...
class foss4g2011_example3_starter(QObject):
    ...
    def initGui(self):
        ...
        self.action = QAction(QIcon(":/plugins/foss4g2011_example3_starter/icon.png"), \
            "Example #3 Starter", self.iface.mainWindow())
        # connect the action to the run method
        QObject.connect(self.action, SIGNAL("triggered()"), self.run)

        # Add toolbar button and menu item
        self.iface.addToolBarIcon(self.action)
        self.iface.addPluginToMenu("&Example #3 Starter", self.action)

    def unload(self):
        # Remove the plugin menu item and icon
        self.iface.removePluginMenu("&Example #3 Starter",self.action)
